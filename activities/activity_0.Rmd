...
# Simple Data Manipulations, Visualizations, & Investingating Warnings
...
`ggplot2` (should have) gave you a warning message. Something like:
...
Apparently - as we can tell from our `facet_wrap()` call - Cambodia does not report this variable to the World Bank.

#### Inspecting Tabularly

Alternatively, we could have used `filter()` and `if_any()` to subset rows which were missing in one of the 3 variables we used in the plot (see below). 
...
dat %>% 
  ...
  filter(
    if_any(
      # function to apply for filtering
      .fns = is.na
      )
  )
```
...
* Pipe this to `drop_na(se_com_durs)` to eliminate countries with missing data in this variable
...
Explain what this code is doing:
    ...
    dat %>% 
      ...
      summarize(
        mean_edu_expendature = mean(se_xpd_totl_gb_zs, na.rm = T)
      )
...
dat %>% 
  ...
  summarize(
    mean_edu_expendature = mean(se_xpd_totl_gb_zs, na.rm = T), 
    sd = sd(se_xpd_totl_gb_zs, na.rm = T),
    se = sd/sqrt(n()), 
