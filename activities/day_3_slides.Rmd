...
set.seed(022023)
sea_split <- initial_split(dat, strata = country_name)
...
```{r}
tree_sizes <- 
  seq(from = 400, to = 3000, by = 200)

mtry_values <- 
  seq(from = 2, to = ncol(sea_testing)-1, 3)
...
```{r, eval = FALSE}
cv_obj <- vfold_cv(sea_training, v = 5, repeats = 2, strata = country_name)
```
...
max_entropy_grid <-
  grid_max_entropy(
    ...
    dials::min_n(range = c(min(node_sizes), max(node_sizes))),
    size = 150
  )
...
tuned_res %>% 
  select(trees, mtry, min_n, mae_mean) %>% 
  #mutate(minimum = factor(if_else(mae_mean == min(mae_mean), 5, 1))) %>% 
  ggplot(
    aes(
      y = trees,
      x = min_n,
  ...
  geom_point(
    data = 
      filter(tuned_res, mae_mean == min(mae_mean)), 
    size = 15,
    shape = 1, 
    color = 'darkred'
    ) + 
  geom_hline(
    data = 
      filter(tuned_res, mae_mean == min(mae_mean)), 
    linetype = 2, 
    aes(yintercept = trees)
  ) + 
    geom_vline(
    data = 
      filter(tuned_res, mae_mean == min(mae_mean)), 
    linetype = 2, 
    aes(xintercept = min_n)
  ) +
  geom_point() +
...
sea_testing %>% 
  transmute(
    !!sym(outcome_next_year),
    prediction_tuned.rf = predict(tuned_rf, .)$prediction
  ) %>% 
  summarize(
    rmse_rf.tuned = 
      rmse(observed_y = !!sym(outcome_next_year), predicted_y = prediction_tuned.rf)
  ) 
